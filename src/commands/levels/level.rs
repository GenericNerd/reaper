...
async fn add_level(
    ...
    let Some(level) = options.get_integer("level") else {
        return Err(ResponseError::Execution(
            "No level provided",
            Some("Please provide levels to add".to_string()),
        ));
    };
    if level <= 0 {
        return Err(ResponseError::Execution(
            "Invalid level provided",
            Some("Please provide a level to add above 0".to_string()),
        ));
    }

    let old_level = ((-25.0 + f64::sqrt((625 + (200 * user_xp)) as f64)) / 100.0).floor() as i64;
    let new_level = old_level + level;
    let new_xp = (50 * (new_level * new_level)) + (25 * new_level);
    let xp = new_xp - user_xp;
...
async fn set_level(
    ...
    let Some(user) = options.get_user("user").into_owned() else {
        return Err(ResponseError::Execution(
            "No user provided",
            Some("Please provide a user to set their level".to_string()),
        ));
    };
    ...
    let Some(level) = options.get_integer("level") else {
        return Err(ResponseError::Execution(
            "No level provided",
            Some("Please provide a level to set".to_string()),
        ));
    };
    ...
    let current_xp = match sqlx::query!(
        "SELECT xp FROM user_xp WHERE guild_id = $1 AND user_id = $2",
        ctx.guild.id.get() as i64,
        user.id.get() as i64
    )
    ...
    let old_level = ((-25.0 + f64::sqrt((625 + (200 * current_xp)) as f64)) / 100.0).floor() as i64;

    if old_level != level {
        handler
            .user_level_up(&ctx.ctx, member, cmd.channel_id, level)
            .await;
    }

    let new_level_xp = (50 * (level * level)) + (25 * level);
    sqlx::query!(
            ...
            .send_message(
                ...
                CreateMessage::new().embed(
                    CreateEmbed::new()
                        .title("Level Set")
                        .description(format!(
                            "<@{}> set <@{}>'s level to {level}",
                            cmd.user.id.get(),
                            user.id.get()
                        ))
                        .footer(CreateEmbedFooter::new(format!(
                            "User {} set user {}'s level to {level}",
                            cmd.user.id.get(),
                            user.id.get()
                        )))
                        .color(0x00ff00),
...
async fn reset_level(
    ...
    let Some(user) = options.get_user("user").into_owned() else {
        return Err(ResponseError::Execution(
            "No user provided",
            Some("Please provide a user to reset their level".to_string()),
        ));
    };
    ...
    ctx.reply(
        ...
        Response::new().embed(
            CreateEmbed::new()
                .title("Level Reset")
                .description(format!("<@{}> has been reset to level 0", user.id.get()))
                .color(0x00ff00),
        ),
    )
    .await
        ...
        CreateCommand::new("level")
            ...
            .add_option(
                CreateCommandOption::new(
                    CommandOptionType::SubCommand,
                    "reset",
                    "Reset someone's level",
                )
                .add_sub_option(
                    CreateCommandOption::new(
                        CommandOptionType::User,
                        "user",
                        "The user who's level to reset",
                    )
                    .required(true),
                ),
            )
            .add_option(
                CreateCommandOption::new(
                    CommandOptionType::SubCommand,
                    "add",
                    "Add to someone's level",
                )
                .add_sub_option(
                    CreateCommandOption::new(
                        CommandOptionType::User,
                        "user",
                        "The user who's level to add to",
                    )
                    .required(true),
                )
                .add_sub_option(
